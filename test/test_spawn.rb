...
module SpawnImplementationTests
  ...
  def test_sanity_of_checking_clone_with_sh
    rd, wr = IO.pipe
    pid = _spawn("sh -c \"exec 2>/dev/null 100<&#{rd.posix_fileno}\" || exit 1", rd => rd)
  ...
  def test_spawn_close_option_with_symbolic_standard_stream_names
    pid = _spawn('sh -c "exec 2>/dev/null 100<&0" || exit 1', :in => :close)
    assert_process_exit_status pid, 1

    pid = _spawn('sh -c "exec 2>/dev/null 101>&1 102>&2" || exit 1',
                 :out => :close, :err => :close)
    assert_process_exit_status pid, 1
  end

  def test_spawn_close_on_standard_stream_io_object
    pid = _spawn('sh -c "exec 2>/dev/null 100<&0" || exit 1', STDIN => :close)
    assert_process_exit_status pid, 1
  ...
  def test_spawn_close_option_with_fd_number
    rd, wr = IO.pipe
    pid = _spawn("sh -c \"exec 2>/dev/null 100<&#{rd.posix_fileno}\" || exit 1", rd.posix_fileno => :close)
    assert_process_exit_status pid, 1
  ...
  def test_spawn_close_option_with_io_object
    rd, wr = IO.pipe
    pid = _spawn("sh -c \"exec 2>/dev/null 100<&#{rd.posix_fileno}\" || exit 1", rd => :close)
    assert_process_exit_status pid, 1
  ...
  def test_spawn_closing_multiple_fds_with_array_keys
    rd, wr = IO.pipe
    pid = _spawn("sh -c \"exec 2>/dev/null 101>&#{wr.posix_fileno}\" || exit 1", [rd, wr, :out] => :close)
    assert_process_exit_status pid, 1
  ensure
    [rd, wr].each { |fd| fd.close rescue nil }
  end
  ...
  def test_explicitly_passing_an_fd_as_open
    rd, wr = IO.pipe
    pid = _spawn("sh -c \"exec 101>&#{wr.posix_fileno}\" || exit 1", wr => wr)
    assert_process_exit_ok pid
  ensure
    [rd, wr].each { |fd| fd.close rescue nil }
  end
